...
async function createUser(username, email, password, age) {
    try {
        ...
        return result.rows[0]; // Return new user data
    }
    catch (error) {
...
// Todo create karna
async function createTodo(user_id, title, description) {
    try {
        const query = `
...
async function getUserTodos(user_id) {
    try {
        const result = await pgClient.query("SELECT * FROM todos WHERE user_id = $1 ORDER BY created_at DESC;", [user_id]);
...
async function updateUser(id, username, age) {
    try {
        const query = `
            UPDATE users
...
async function getUserWithTodoCount() {
    try {
